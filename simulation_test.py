...
def main():
    node1 = get_node_info("127.0.0.1:5001")
    node2 = get_node_info("127.0.0.1:5002")
    node3 = get_node_info("127.0.0.1:5003")
    ...
    print node1_seeds
    bootstrap("127.0.0.1:5001", node1_seeds)

    node2_seeds = [
        {"node_id": node1["node_id"], "ip": node1["ip"], "port": node1["port"]},
        {"node_id": node3["node_id"], "ip": node3["ip"], "port": node3["port"]}
    ]
    bootstrap("127.0.0.1:5002", node2_seeds)

    node3_seeds = [
        {"node_id": node2["node_id"], "ip": node2["ip"], "port": node2["port"]},
        {"node_id": node1["node_id"], "ip": node1["ip"], "port": node1["port"]}
    ]
    bootstrap("127.0.0.1:5003", node3_seeds)
    ...
    while True:
        ...
        if node1_balance > 0:
            ...
            simulate_tx("127.0.0.1:5001", node1_wallet, node3_wallet, amount)
            time.sleep(random.randint(15, 20))
        #
        # node2 发送给node1 node3
        node2_balance = get_balance("127.0.0.1:5002", node2_wallet)
        ...
        if node2_balance > 0:
            ...
            simulate_tx("127.0.0.1:5002", node2_wallet, node1_wallet, amount)
            time.sleep(random.randint(15, 20))
        ...
        if node2_balance > 0:
            ...
            simulate_tx("127.0.0.1:5002", node2_wallet, node3_wallet, amount)
            time.sleep(random.randint(15, 20))
        ...
        if node3_balance > 0:
            ...
            simulate_tx("127.0.0.1:5003", node3_wallet, node2_wallet, amount)
            time.sleep(random.randint(15, 20))
